...
const (
	...
	varModelPkgPathNameKey     = "VarModelPkgPathName"
	varMysqlImpPkgPathNameKey  = "VarMysqlImpPkgPathNameKey"
	varMysqlImpPkgNameKey      = "VarMysqlImpPathNameKey"
	varinstanceFuncKey         = "VarinstanceFuncKey"
)
...
func (g *generator) makeDao() {
	...
	for _, m := range models {
		...
		if len(g.opts.daoMysqlImpPkgPath) > 0 {
			g.makeModelExternalCreatTalbeDao(m)
		} else {
...
func (g *generator) makeModelExternalCreatTalbeDao(m *model) {
	...
	g.opts.daoMysqlImpPkgPath = g.loadPackage().Module.Path + g.opts.daoMysqlImpPkgPath
	replaces := make(map[string]string)
	replaces[varDaoClassNameKey] = m.daoClassName
	replaces[varDaoPrefixNameKey] = m.daoPrefixName
	...
	replaces[varModelPackageNameKey] = m.modelPkgName
	replaces[varModelPkgPathNameKey] = m.modelPkgPath
	replaces[varModelClassNameKey] = m.modelClassName
	replaces[varMysqlImpPkgPathNameKey] = g.opts.daoMysqlImpPkgPath
	replaces[varMysqlImpPkgNameKey] = defaultModelMysqlpkgName
	replaces[varinstanceFuncKey] = g.opts.instanceFunc
	err = doWrite(file, template.ExternalTemplateCreteTable, replaces)
	if err != nil {
		log.Fatal(err)
	}
}

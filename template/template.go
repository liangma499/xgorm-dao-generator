...
`
const ExternalTemplateCreteTable = `
package ${VarDaoPackageName}
...
import (
	...
	"sync"
	"${VarDaoPackagePath}/internal"
	${VarModelPackageName} "${VarModelPkgPathName}"
	${VarMysqlImpPathNameKey} "${VarMysqlImpPkgPathNameKey}"
	"gorm.io/gorm"
)
...
var (
	once     sync.Once
	instance *${VarDaoClassName}
)

func Instance() *${VarDaoClassName} {
	once.Do(func() {
		instance = New${VarDaoClassName}(${VarMysqlImpPathNameKey}.${VarinstanceFuncKey}())
	})
	return instance
}
func (dao *${VarDaoClassName}) CreateTable() error {
	err := dao.Database.WithContext(context.Background()).AutoMigrate(&${VarModelPackageName}.${VarModelClassName}{})
	if err != nil {
		panic(err)
	}
	return nil
...
type ${VarDaoPrefixName}FilterFunc func(cols *${VarDaoPrefixName}Columns) any
type ${VarDaoPrefixName}UpdateFunc func(cols *${VarDaoPrefixName}Columns) any
